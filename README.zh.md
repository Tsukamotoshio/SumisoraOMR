...
1. 将五线谱文件（`.pdf`、`.png`、`.jpg`）放入 `Input` 文件夹，或直接拖入
   左侧文件侧边栏。
...
点击侧边栏标题栏中的导出按钮，即可批量复制到指定目录。点击顶栏的**简谱编辑**
按钮可打开当前预览的文件进行编辑。
...
移调引擎读取 `xml-scores/` 中的 MusicXML 文件，将其渲染为不同调性。支持三种模式：

- **按半音** — 直接输入半音偏移量。
- **按音程** — 选择具体音程（纯四度、大三度等）及方向。
- **全音移调** — 按音阶度数在调内移动。

任意参数变化时自动触发预览。结果可导出为简谱 PDF 或五线谱 PDF。
