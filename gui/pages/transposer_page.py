...
# 常用调名列表
_KEYS = ['C', 'G', 'D', 'A', 'E', 'B', 'F#', 'F', 'Bb', 'Eb', 'Ab', 'Db']
...
class TransposerPage(ft.Column):
    ...
    async def _pick_file_async(self) -> None:
        ...
        if suffix in ('.mxl', '.musicxml', '.xml'):
            ...
            threading.Thread(target=self._render_orig, daemon=True).start()
    ...
    def _auto_detect_async(self) -> None:
        try:
    ...
    def _on_transpose(self, _e) -> None:
        if self._orig_mxl is None:
            self._set_status('请先在首页转换文件。')
            return
        threading.Thread(target=self._transpose_async, daemon=True).start()

    def _transpose_async(self) -> None:
        self._set_busy(True)
        try:
            ...
            # 渲染移调后预览
            threading.Thread(target=self._render_transposed, daemon=True).start()
        except Exception as exc:
    ...
    def _export_async(self) -> None:
        self._set_busy(True)
        try:
            from core.renderer import generate_jianpu_pdf_from_mxl
            from core.utils import get_app_base_dir
            import tempfile

            base = get_app_base_dir() / 'Output'
            base.mkdir(exist_ok=True)
            out_pdf = base / f'{self._transposed_mxl.stem}_jianpu.pdf'
            tmp = get_app_base_dir() / 'build' / f'_trans_export_{self._transposed_mxl.stem}'
            tmp.mkdir(exist_ok=True)

            ok = generate_jianpu_pdf_from_mxl(
                self._transposed_mxl, out_pdf, tmp, None,
                preferred_title=self._transposed_mxl.stem,
            )
            if ok:
                self._set_status(f'导出完成 → {out_pdf.name}')
                self._state.output_pdf = out_pdf
            else:
                self._set_status('导出失败，请查看日志。')
        except Exception as exc:
    ...
    def _render_transposed(self) -> None:
        if self._transposed_mxl is None:
            return
    ...
    def _mxl_to_preview_pdf(self, mxl_path: Path, suffix: str) -> Optional[Path]:
        ...
        try:
            ...
            # 回退：简谱渲染
            from core.renderer import generate_jianpu_pdf_from_mxl
            ok = generate_jianpu_pdf_from_mxl(mxl_path, out, tmp, None, preferred_title=mxl_path.stem)
