...
[![版本: v0.3.4](https://img.shields.io/badge/Version-v0.3.4-green.svg)]()
...
点击侧边栏标题栏中的导出按钮，即可批量复制到指定目录。点击**重新渲染**按钮可
直接从 `.jianpu.txt` 源文件重新生成当前 PDF（无需重跑识别）。点击**编辑**按钮
可打开当前预览的文件进行简谱校对。
...
- **多声部支持**：简谱与五线谱预览均支持最多 4 个独立声部，各声部独立成行；超过 4 声部或极密集的和弦可能丢失部分音符。
- **不支持歌词输出**：仅输出音符，不含歌词。
