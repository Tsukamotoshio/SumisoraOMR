...
if errorlevel 1 (
    ...
    echo [ERROR] 未找�?GitHub CLI ^(gh^)�?    echo 请先安装：winget install --id GitHub.cli -e
...
gh release create "%TAG%" ^
    ...
    --notes "### 简谱转换工�?v%APP_VERSION%^^  ^^**安装包（推荐�?*：下�?%APP_NAME%-Setup-%APP_VERSION%.exe 运行安装向导即可。^^**便携�?*：解�?%APP_NAME%-Portable-%APP_VERSION%.zip 后直接运�?ConvertTool.exe�? ^

    --draft

if errorlevel 1 (

    echo [WARN] Release 可能已存在，尝试仅上传文�?..
...
echo [OK] 已上传至 GitHub Release %TAG%（草稿状态）�?echo [INFO] 请前往 GitHub 检查发布说明后手动发布�?echo        gh release edit "%TAG%" --draft=false

echo        或在 GitHub 网页上点�?Publish release
