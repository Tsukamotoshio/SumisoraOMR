...
class ScorePreviewPage(ft.Row):
    ...
    def _open_midi(self, midi_path: Path) -> None:
        ...
        try:
            ...
            self._snack(t("common.midi_open_failed", exc=exc))
    ...
    def _confirm_generate_midi(self, mxl_path: Path, midi_path: Path) -> None:
        ...
        self.page.show_dialog(ft.AlertDialog(  # type: ignore[attr-defined]
            modal=True,
            title=ft.Text(t("score_preview.midi_generate_title"), size=15, font_family=FONT_EMPHASIS),
            content=ft.Text(
                t("score_preview.midi_generate_body", name=midi_path.name),
                size=13,
                color=ft.Colors.ON_SURFACE,
            ),
            actions=[
                ft.TextButton(t("common.cancel"), on_click=_cancel),
                ft.FilledButton(t("score_preview.midi_generate_confirm"), on_click=_do),
            ],
            actions_alignment=ft.MainAxisAlignment.END,
        ))
    ...
    def _generate_and_play_midi(self, mxl_path: Path, midi_path: Path) -> None:
        ...
        def _work() -> None:
            try:
                ...
                ok = False
                self._set_status(t("score_preview.midi_generate_failed_exc", exc=exc))
            finally:
                self._set_busy(False)
            if ok and midi_path.exists():
                self._set_status(t("score_preview.midi_generated", name=midi_path.name))
                # os.startfile 须在主线程调用，调度回事件循环
                p = self.page
                if p is not None:
                    p.loop.call_soon_threadsafe(self._open_midi, midi_path)  # type: ignore[attr-defined]
            else:
                self._set_status(t("score_preview.midi_generate_failed"))

        self._set_busy(True)
        self._set_status(t("score_preview.midi_generating"))
